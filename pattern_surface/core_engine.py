...
def translated_carrier(triangle, axis, offset):
    from .patterns.diamond.lattice import translated_carrier as implementation
    return implementation(triangle, axis, offset)

def periodic_carriers(payload, carriers):
    from .patterns.diamond.lattice import periodic_carriers as implementation
    return implementation(payload, carriers, periodic_axis_records(payload))

def canonical_periodic_representative(payload, canonical):
    from .patterns.diamond.lattice import canonical_periodic_representative as implementation
    return implementation(payload, canonical, periodic_axis_records(payload))

def canonical_triangles(bounds, extra=True, diamond_height=GRID_HEIGHT,
                        diamond_side=None, origin_x=0.0):
    """Compatibility facade for the Diamond-owned lattice generator."""
    from .patterns.diamond.lattice import canonical_triangles as implementation
    return implementation(bounds, extra, diamond_height, diamond_side, origin_x)
