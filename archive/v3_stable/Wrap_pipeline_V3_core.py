"""Shared geometric core for the Wrap Faces V3 FreeCAD macros.
...
SCHEMA = "WRAP_CARRIER_V3"
WRAP_PREFIX = "DiamondSurfaceWrap_V3"
FULL_PREFIX = "DiamondPatternFullFromWrap_V3"
CUT_PREFIX = "DiamondPatternCutFromWrap_V3"
GRID_HEIGHT = 12.0
...
def selected_faces():
    ...
    if not entries:
        fail("Selecione uma ou mais faces antes de executar Wrap Faces V3.")
...
def outward(entry):
    ...
    if not votes or any(value != votes[0] for value in votes):
        fail("Direcao externa ambigua em {}. O Wrap V3 foi abortado.".format(entry["sub"]))
    return votes[0]
...
def neighbor_transform_candidates(placed, target, placed_edge, target_edge):
    ...
    if curved_seam and abs(dl - sl) > 0.05:
        console("wrap_v3: emenda_curva_sem_escala {} <-> {} sl={:.4f} dl={:.4f}".format(
...
def align_connected_normals(entries, graph):
    ...
    if flips:
        console("wrap_v3: normais_conectadas_invertidas={}".format(flips))
        ...
        while queue:
            ...
            for neighbor_index, current_edge, neighbor_edge in graph[current["index"]]:
                ...
                if len(constraints) > 1:
                    ...
                    if ignored:
                        console("wrap_v3: emenda_atlas_principal={} <-> {}; emendas_extras_ignoradas={}".format(
                ...
                if error is None or error > limit:
                    fail("Carrier V3 nao conseguiu posicionar a emenda {} <-> {} (erro {}).".format(
                        primary_placed["sub"], neighbor["sub"], error))
                if error > 0.05:
                    warn("Carrier V3 aceitou emenda curva aproximada {} <-> {} erro={:.4f} limite={:.4f}.".format(
...
def add_logical_seam_overrides(entries, graph, atlas_pairs=None):
    ...
    for entry in entries:
        for neighbor_index, edge, other_edge in graph[entry["index"]]:
            ...
            added += 1
    if added:
        console("wrap_v3: emendas_logicas_fixadas={}".format(added))
...
def conforming_carrier(entries):
    ...
    console("wrap_v3: carrier_parametrico={} longest={:.4f} sag={:.4f}".format(
...
def validate_logical_seams(entries, graph, atlas_pairs=None):
    ...
    for entry in entries:
        for neighbor_index, edge, other_edge in graph[entry["index"]]:
            ...
            if error > limit:
                fail("Emenda logica V3 {} <-> {} divergiu {:.4f} mm.".format(
                    entry["sub"], neighbor["sub"], error))
            if error > 0.05:
                warn("Emenda logica V3 curva aproximada {} <-> {} erro={:.4f} limite={:.4f}.".format(
                    entry["sub"], neighbor["sub"], error, limit))
    console("wrap_v3: emendas_logicas={} erro_max={:.4f}".format(len(checked), maximum))
    return maximum
...
def close_periodic_component(vertices, triangle_indices, face_ids, seam_pairs):
    ...
    width = max_x - min_x
    if width <= 1.0e-8:
        fail("Componente periodico V3 sem largura logica.")
    columns = max(1, int(round(width / GRID_SIDE)))
    target = columns * GRID_SIDE
    adjustment = (target - width) / width
    if abs(adjustment) > 0.05:
        fail("Fechamento periodico V3 exige ajuste de {:.2f}% (limite 5%).".format(abs(adjustment) * 100.0))
    scale = target / width
    for item in used:
...
def unfold_carrier(triangles, groups, graph):
    ...
    for group in groups:
        ...
        if not members:
            fail("Componente do carrier V3 sem triangulos.")
        ...
        if qc is None:
            fail("Triangulo ancora degenerado no carrier V3.")
        ...
        while queue:
            ...
            for neighbor_index in adjacency[current_index]:
                ...
                if candidate is None:
                    fail("Falha ao desdobrar triangulo vizinho no carrier V3.")
        ...
        if missing:
            fail("Tessellacao V3 nao compartilha nos BReps: {} triangulos desconectados.".format(len(missing)))
    ...
    console("wrap_v3: erro_fechamento_logico_max={:.4f}".format(maximum_closure_error))
...
def create_wrap():
    ...
    if doc is None:
        fail("Abra um documento antes de executar Wrap Faces V3.")
    ...
    if not triangles:
        fail("Carrier V3 nao gerou triangulos internos.")
    ...
    add_string(run, "WrapVersion", "Wrap Faces V3", "Wrap Faces V3")
    add_string(run, "WrapAlgorithm", SCHEMA, "Wrap Faces V3")
    add_string(run, "WrapReady", "True", "Wrap Faces V3")
    add_string(run, "WrapSourceFaces", ";".join("{}|{}".format(e["object"].Name, e["sub"]) for e in entries), "Wrap Faces V3")
    add_string(run, "WrapAdjacency", ";".join("{}>{}".format(a + 1, b + 1) for a, b in shared), "Wrap Faces V3")
    add_chunks(run, "WrapCarrierChunks", payload, "Wrap Faces V3")
    ...
    add_string(preview, "WrapParentRun", run.Name, "Wrap Faces V3")
    ...
    console("wrap_v3: run={} faces={} carrier_triangles={}".format(name, len(entries), len(triangles)))
    return run


def resolve_wrap_selection(doc):
    for obj in Gui.Selection.getSelection():
        if getattr(obj, "WrapAlgorithm", "") == SCHEMA:
            return obj
        ...
        if parent:
            ...
            if run is not None and getattr(run, "WrapAlgorithm", "") == SCHEMA:
                return run
    fail("Selecione o objeto DiamondSurfaceWrap_V3_Run_... antes de executar esta macro.")
...
def canonical_shell_solid(canonical, context, apex_override=None):
    loop = []
    ...
    if center_value is None or center_value[1] is None:
        return None, None
    apex = apex_override if apex_override is not None else center_value[0] + center_value[1] * RELIEF
...
def curved_shell_pyramid_solid(canonical, context, apex_override=None):
...
def curved_height_mapped_solid(canonical, context, apex_override=None, source_solids=None):
    ...
    for i in range(count + 1):
        for j in range(count + 1 - i):
            ...
            top_nodes[(i, j)] = base + normal * (RELIEF * weight)
    ...
    if center_normal is None:
        return None, None
    apex = apex_override if apex_override is not None else center_value[0] + center_normal * RELIEF
...
def choose_outside_apex(base_center, apex_normal, source_solids):
    if not source_solids:
        return base_center + apex_normal * RELIEF
    plus = base_center + apex_normal * RELIEF
    minus = base_center - apex_normal * RELIEF
    plus_inside = False
    minus_inside = False
    checked = False
...
def canonical_lattice_solid(canonical, context, apex_override=None, source_solids=None):
...
def curved_lattice_pyramid_solid(canonical, context, apex_override=None, source_solids=None):
    ...
    def side_point(edge_q, ratio):
        ...
        if normal is None:
            return None
        return mapped[0] + normal * (RELIEF * ratio - CONTACT * (1.0 - ratio))
...
def curved_row_pyramid_solid(canonical, context, apex_override=None, source_solids=None):
    ...
    fallback only.
    """
    solid, apex = curved_lattice_pyramid_solid(canonical, context, apex_override, source_solids)
    if solid is not None:
        return solid, apex
    solid, apex = curved_shell_pyramid_solid(canonical, context, apex_override)
    if solid is not None:
        return solid, apex
    solid, apex = canonical_shell_solid(canonical, context, apex_override)
    if solid is not None:
        return solid, apex
    return curved_height_mapped_solid(canonical, context, apex_override, source_solids)


def build_cells(payload, include_ghost, allowed_ids=None, apex_records=None, source_shapes=None):
    ...
    for triangle_id, canonical in canonical_triangles(payload["bounds"], extra=include_ghost):
        ...
        if any(fragment["carrier"].get("curved", False) for fragment in fragments):
            solid, apex = curved_row_pyramid_solid(canonical, context, apex_override, source_solids)
        if solid is None:
            solid, apex = canonical_lattice_solid(canonical, context, apex_override, source_solids)
        if solid is None:
...
def exact_face_cut_envelope(entry):
    ...
    if is_valid_shape(cached):
        return cached
    front_depth = RELIEF + CUT_FRONT_MARGIN
    ...
    try:
        ...
        warn("cut_v3: envelope_offset_falhou face={} erro={}".format(entry.get("index", "?"), exc))
    ...
    try:
        ...
        warn("cut_v3: envelope_plano_falhou face={} erro={}".format(entry.get("index", "?"), exc))
    return None
...
def fused_cut_envelopes(envelopes):
    ...
    for envelope in envelopes[1:]:
        try:
            ...
            warn("cut_v3: envelope_fuse_falhou erro={}".format(exc))
            return None
    try:
...
def physical_cut_piece(cell, combined_envelope, envelopes, index):
    ...
    if combined_envelope is not None:
        try:
            ...
            warn("cut_v3: booleano_combinado_falhou celula={} erro={}".format(index, exc))
    if not pieces:
        for envelope in envelopes:
            try:
                ...
                warn("cut_v3: booleano_face_falhou celula={} erro={}".format(index, exc))
    return pieces


def build_cut_cells_from_full(doc, payload, pattern, cell_payload):
    """Trim Full solids by the selected face envelopes, preserving Full geometry."""
    entries = hydrate_entries(doc, payload)
    envelopes = [exact_face_cut_envelope(entry) for entry in entries]
    ...
    if not full_solids or len(full_solids) != len(full_records):
        warn("cut_v3: full_solidos_registros_incompativeis solidos={} registros={}".format(
...
def create_full_pattern():
    doc = App.ActiveDocument
    if doc is None:
        fail("Abra um documento antes de executar o Pattern Full V3.")
    wrap = resolve_wrap_selection(doc)
    payload = load_chunks(wrap, "WrapCarrierChunks")
    if payload.get("schema") != SCHEMA:
        fail("Carrier selecionado nao e V3.")
    solids, records, rejected = build_cells(payload, True, source_shapes=source_solids_by_face(doc, payload))
    if not solids:
        fail("Pattern Full V3 nao gerou solidos validos.")
    name = next_name(doc, FULL_PREFIX)
    run = doc.addObject("PartDesign::Feature", name)
    run.Label = name + " Solid"
    run.Shape = Part.makeCompound(solids)
    add_string(run, "DiamondPatternVersion", "Pattern Full From Wrap V3", "Diamond Pattern V3")
    add_string(run, "DiamondPatternAlgorithm", "WRAP_CARRIER_V3_FULL", "Diamond Pattern V3")
    add_string(run, "DiamondPatternWrapSource", wrap.Name, "Diamond Pattern V3")
    add_string(run, "DiamondPatternRejected", ";".join(rejected), "Diamond Pattern V3")
    add_chunks(run, "DiamondPatternCellChunks", {"cells": records}, "Diamond Pattern V3")
    doc.recompute()
    console("pattern_full_v3: run={} solids={} rejected={}".format(name, len(solids), len(rejected)))
    if rejected:
        warn("pattern_full_v3: celulas_rejeitadas={}".format(",".join(rejected)))
    return run
...
def resolve_cut_selection(doc):
    ...
    for obj in Gui.Selection.getSelection():
        if getattr(obj, "WrapAlgorithm", "") == SCHEMA:
            wrap = obj
        if getattr(obj, "DiamondPatternAlgorithm", "") == "WRAP_CARRIER_V3_FULL":
            pattern = obj
    if wrap is None or pattern is None:
        fail("Selecione o Pattern Full V3 e o Wrap V3 correspondentes.")
    if getattr(pattern, "DiamondPatternWrapSource", "") != wrap.Name:
        fail("O Pattern Full V3 nao pertence ao Wrap V3 selecionado.")
    return wrap, pattern
...
def create_cut():
    doc = App.ActiveDocument
    if doc is None:
        fail("Abra um documento antes de executar o Cut V3.")
    ...
    solids, records, rejected = build_cut_cells_from_full(doc, payload, pattern, cell_payload)
    algorithm = "WRAP_CARRIER_V3_PHYSICAL_NORMAL_CUT"
    if not solids:
        warn("cut_v3: corte_fisico_falhou; tentando_rebuild_antigo")
        solids, records, rejected = build_cut_cells(payload, allowed, apex)
        algorithm = "WRAP_CARRIER_V3_CUT_REBUILD_FALLBACK"
    if not solids:
        fail("Cut V3 nao gerou solidos validos.")
    name = next_name(doc, CUT_PREFIX)
    run = doc.addObject("PartDesign::Feature", name)
    run.Label = name + " Solid"
    run.Shape = Part.makeCompound(solids)
    add_string(run, "DiamondPatternVersion", "Cut From Wrap V3", "Diamond Pattern V3")
    add_string(run, "DiamondPatternAlgorithm", algorithm, "Diamond Pattern V3")
    add_string(run, "DiamondPatternWrapSource", wrap.Name, "Diamond Pattern V3")
    add_string(run, "DiamondPatternFullSource", pattern.Name, "Diamond Pattern V3")
    add_string(run, "DiamondPatternRejected", ";".join(rejected), "Diamond Pattern V3")
    add_chunks(run, "DiamondPatternCellChunks", {"cells": records}, "Diamond Pattern V3")
    doc.recompute()
    console("cut_v3: run={} algoritmo={} solids={} rejected={}".format(name, algorithm, len(solids), len(rejected)))
    if rejected:
        warn("cut_v3: celulas_rejeitadas={}".format(",".join(rejected)))
    return run
